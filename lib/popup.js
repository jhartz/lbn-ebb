        ...
        document.getElementById("impress").addEventListener("impress:stepenter", function (event) {
            ...
            if (active.classList.contains("scroll-me") && !runningload) {
                timerequests.push(function (timeavailable) {
                    ...
                    style.type = "text/css";
                    var time = (timeavailable - beginningwait) - 1;
                    ...
                    setTimeout(function () {
                        ...
                        }, (timeavailable * 1000) - beginningwait);
                    }, beginningwait);
                });
                send("timerequest");
            }
...
function fontSize(step) {
    ...
    while (cur < 200 && elem.scrollHeight <= elem.clientHeight && elem.scrollWidth <= elem.clientWidth) {
        cur += 2;
        elem.style.fontSize = cur + "px";
    }
    cur -= 2;
    ...
    if (elem.scrollHeight > elem.clientHeight) {
        send("enableExtraTime", {step: step, time: Math.round((elem.scrollHeight - elem.clientHeight) / 70)});
