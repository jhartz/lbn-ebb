...
function msg(cmd, data) {
    switch (cmd) {
        ...
        case "nextload":
            var shouldbe = data;
            var current = shouldbe - 1;
            if (document.getElementById("step-" + shouldbe)) {
                while (document.getElementById("step-" + shouldbe) && document.getElementById("step-" + shouldbe).classList.contains("no-font-adjust")) {
                    document.getElementById("step-" + shouldbe).classList.remove("unloaded");
                    shouldbe++;
                }
                if (!document.getElementById("step-" + shouldbe)) shouldbe = 1;
                if (shouldbe == current) {
                    // We're probably stuck on the first one
                    if (api) api.next();
                } else {
                    if (api) api.goto(shouldbe - 1, 400);
                }
            } else {
                if (api) api.next();
            }
        ...
        case "timerequest":
            ...
            ebb.util.forEach(timerequests, function (callback) {
                a++;
                callback(data);
                if (a == b) timerequests = [];
            });
...
function makeElem(seq) {
    ...
    elem.className = "step unloaded " + (seq.className || "");
    if (seq.data) {
...
function load(newsequence) {
    ...
    if (initialized && sequence && !started) {
        ...
        document.getElementById("impress").addEventListener("impress:stepenter", function (event) {
            ...
            if (active.classList.contains("unloaded")) active.classList.remove("unloaded");
        ...
        document.getElementById("impress").addEventListener("impress:stepleave", function (event) {
            var step, direction;
            if (event.detail) {
                if (event.detail.nextStep) step = Number(event.detail.nextStep.substring(5));
                if (event.detail.direction) direction = event.detail.direction;
            }
            send("stepleave", {
                doneLoading: !document.getElementsByClassName("unloaded").length,
                nextStep: step,
                direction: direction
            });
        }, false);
