...
ebb.control = {
    ...
    getOption: function (pluginname, option) {
        return localStorage.getItem("_OPTION_" + pluginname + "_" + option.name) || option.value || (option.type && ebb.control.OPTION_DEFAULTS[option.type]) || "";
    },
    ...
    replace_variables: function (content, plugin) {
        if (plugin.options) {
            forEach(plugin.options, function (option) {
                if (option.variable) {
                    while (content.indexOf(option.variable) != -1) {
                        content = content.replace(option.variable, ebb.control.getOption(plugin.name, option));
                    }
                }
            });
        }
    ...
    render: function (plugin) {
        ...
        var pusher = function (content, position) {
            sequence.push({
                name: plugin.name,
                description: plugin.description ? escHTML(plugin.description, true) : content,
                content: content,
        ...
        if (plugin.type == "html") {
            if (typeof plugin.content == "string") {
                pusher(ebb.control.replace_variables(plugin.content, plugin));
            } else {
                var left = -950;
                forEach(plugin.content, function (content) {
                    if (content) {
                        left += 950;
                        pusher(ebb.control.replace_variables(content, plugin), {
                            x: left,
                            y: ebb.control.topspace
                        });
                    }
            ...
            if (typeof plugin.content == "string") {
                var left = -950;
                var allcontent = ebb.control.filter(ebb.control.replace_variables(plugin.content, plugin));
                forEach(allcontent.split("<br><br>"), function (content) {
                    ...
                    if (content) {
                        ...
                        if (plugin.slidetitle) {
                            content = '<div class="title">' + escHTML(plugin.slidetitle) + '</div>' + content;
                        }
                        pusher(content, {
                            x: left,
                            y: ebb.control.topspace
            ...
            } else if (typeof plugin.content == "object") {
                var left = -950;
                var content = '';
                for (var prop in plugin.content) {
                    if (plugin.content.hasOwnProperty(prop)) {
                        if (typeof plugin.content[prop] == "string") {
                            // It wants a variable turned into an array
                            forEach(plugin.options, function (option) {
                                if (plugin.content[prop] == option.variable) {
                                    var vals = [];
                                    forEach(ebb.control.getOption(plugin.name, option).split("\n"), function (val) {
                                        if (val && val.trim()) {
                                            vals.push(val.trim());
                                        }
                                    });
                                    plugin.content[prop] = vals;
                                }
                            });
                        }
                        content = '';
                        content += '<div class="title">' + escHTML(prop) + '</div>';
                        content += '<ul>';
                        forEach(plugin.content[prop], function (item) {
                            content += '<li>' + escHTML(item) + '</li>';
                        });
                        content += '</ul>';
                        left += 950;
                        pusher(ebb.control.replace_variables(content, plugin), {
    ...
    go: function () {
        ...
        forEach(sequence, function (seq) {
            ...
            content += 'Time: <input type="number" class="control_slidetime" size="3" value="' + seq.time + '" min="1" max="60"> seconds';
            content += '<div class="pretty_title" id="control_title_step' + step + '">' + seq.description + '</div>';
            content += '</td>';
        });
        content += '</tr>';
