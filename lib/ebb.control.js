...
ebb.control = {
    ...
    load: function () {
        ...
        var emptyTextNodes = [];
        ebb.util.forEach(document.querySelectorAll("#header > ul"), function (elem) {
            ebb.util.forEach(elem.childNodes, function (child) {
                if (child.nodeName.toLowerCase() != "li") {
                    emptyTextNodes.push([child, elem]);
                }
            });
        });
        ebb.util.forEach(emptyTextNodes, function (item) {
    ...
    getOption: function (pluginname, option) {
        ...
        switch (option.type) {
            case "text":
                varcontent = ebb.util.escHTML(varcontent);
                break;
            case "textarea":
                varcontent = ebb.util.escHTML(varcontent, true);
                break;
            case "formattedtextarea":
                varcontent = ebb.util.filter(varcontent);
                break;
        }
        return varcontent;
    },
        ...
        switch (cmd) {
            case "ready":
                if (ebb.control.started) {
                    ...
                    ebb.util.forEach(ebb.control.update_intervals, function (i) {
            ...
            case "stepleave":
                if (ebb.control.runningload) {
                    ...
                    ebb.control.stopTimer();
                    ebb.util.forEach(document.getElementsByClassName("active"), function (item) {
    ...
    render: function (plugin) {
        ...
        if (Array.isArray(plugin.content)) {
            var left = -950;
            ebb.util.forEach(plugin.content, function (content) {
                if (content) {
                    left += 950;
            ...
            ebb.util.forEach(allcontent.split("<br><br>"), function (varcontent) {
    ...
    go: function (startAfterLoad) {
        ...
        var content = '';
        var step = 0;
        ebb.util.forEach(sequence, function (seq) {
        ...
        var add_cbox_event = function (checkbox) {
            checkbox.addEventListener("change", function () {
                ...
                ebb.util.forEach(document.getElementsByClassName("control_slideenabled"), function (cbox) {
    ...
    setDefaultTime: function (basetime) {
        ...
        document.getElementById("control_basetime").value = ebb.control.DEFAULT_TIME;
        ebb.util.forEach(document.getElementsByClassName("control_slidetime"), function (slidetime) {
    ...
    setAutoDisable: function (hour, minute) {
        ...
        ebb.control.autodisable.timeout = setTimeout(function () {
            ebb.control.cancelAutoDisable();
            ebb.util.forEach(document.getElementsByClassName("control_slideautodisable"), function (elem) {
        ...
        ebb.util.forEach(document.getElementsByClassName("control_slideautodisable_container"), function (elem) {
